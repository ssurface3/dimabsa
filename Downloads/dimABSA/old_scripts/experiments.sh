...
EXPERIMENTS=(
    ...
    # "Qwen/Qwen3-Embedding-0.6B 13 1 2e-5"
    # "roberta-large 4 4 1e-5"

    # "jhu-clsp/mmBERT-base 16 1 2e-5"
    "RobroKools/vad-bert 16 1 2e-5"
)
...
do
    ...
    python train.py \
        ...
        --data_path "$DATA_DIR/eng_laptop_train_alltasks.jsonl" \
        --output_dir "$EXP_ID" \
        --epochs $EPOCHS \
        --batch_size $BS \
        --grad_accum $ACCUM \
    ...
    python predict.py \
        ...
        --test_data "$DATA_DIR/eng_laptop_dev_task1.jsonl" \
    ...
    # rm $ZIP_NAME
    # rm -rf "models/$EXP_ID" 
    echo "Cleaning up model weights to free space..."
    rm -rf "models/$EXP_ID"
