...
def gen_sub(model_path, test_data, output_file=None, output_dir="results", batch_size=32, max_len=128):
    ...
    try:
        ...
        model = TwoheadModel.from_pretrained(model_path, config=config)
        model.to(device)
        model.eval()
    ...
    # Process just the single file passed in argument
    file_path = test_data
    
    raw_data = parse_jsonl(file_path)
    dataset = InferenceDataset(raw_data, tokenizer, max_len)
    ...
    if output_file:
        ...
        json_output_path = os.path.join(output_dir, f"pred_{base_name}.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_data", "--test_file", dest="test_data", type=str, default="data/test.jsonl")
